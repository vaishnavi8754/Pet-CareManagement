...
const PostPetSection = () => {
  ...
  const [fileName, setFileName] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [category, setCategory] = useState("home"); // New state for category selection
  const [daysAtLocation, setDaysAtLocation] = useState(""); // New state for homeless pet days
  ...
  const handleSubmit = async (e) => {
    ...
    if (category === "home" && (!name || !age || !area || !justification || !email || !phone || !fileName || type === "None" || ageError)) {
      setFormError(true);
      return;
    }

    if (category === "homeless" && (!name || !area || !fileName || type === "None" || !daysAtLocation || !email || !phone)) {
      setFormError(true);
      return;
    }
    ...
    if (category === "home") {
      ...
      formData.append("daysAtLocation", daysAtLocation);
    }
      ...
      <form onSubmit={handleSubmit} encType="multipart/form-data">
        <div className="input-box">
          ...
          <select value={category} onChange={(e) => setCategory(e.target.value)}>
            <option value="home">Home Pet</option>
            <option value="homeless">Homeless Pet</option>
          </select>
        </div>
        ...
        {category === "homeless" && (
          <>
            ...
            <div className="input-box">
              <label>How many days at the location:</label>
              <input
                type="text"
                value={daysAtLocation}
                onChange={(e) => setDaysAtLocation(e.target.value)}
              />
            </div>
          </>
        )}
